...
\begin{abstract}
This report documents my reproduction and extension of the autonomous Maxwell demon model by Mandal \emph{et al.}\cite{mandal2013refrigerator}, a minimal device that rectifies thermal transitions while processing information on a moving tape. First, the original two-state ``Maxwell refrigerator'' is numerically reproduced using a Gillespie simulation, and its known limits and phase behavior are recovered. Second, I demonstrate an alternative route to stronger refrigeration without slowing the tape: stacking multiple demons in series, where increasing the number of demons $K$ mimics increasing the interaction time $\tau$ for a single demon. Third, the framework is generalized to demons with $n>2$ internal states arranged as energy ladders, clarifying how model choices about energy scales and transition parametrizations affect the interpretation of $\Delta E$. Finally, I introduce a two-bit interacting demon that processes bit-pairs and can actively reshape correlations on the tape, extending the information-thermodynamics interplay beyond independent bits.
\end{abstract}
\maketitle
...
To conclude this section, although varying $\tau$ and $\gamma$ reveals how the two-state demon approaches its ceiling, meaningful performance improvements at fixed reservoir parameters motivate redesigns of the architecture. Stacking is one such redesign that preserves the same microscopic rules while changing the physical realization.
...
\begin{figure}[H]
  ...
  \caption{A low-$\Delta E$ ladder example computed by holding $T_H$ and $T_C$ fixed while shrinking the per-rung energy spacing. This changes the underlying dimensionless biases $\sigma_k$ and $\omega_k$ (because $\tanh(\beta\Delta E_k/2)$ depends on $\Delta E_k$), and therefore must be interpreted as a different physical parameter regime rather than a direct ``same-$\sigma,\omega$'' comparison.}
  \label{fig:lowdeltaE}
\end{figure}

At this point, it becomes important to be explicit about what is meant by ``keeping the parameters the same'' in an $n$-state ladder. One can either (i) hold reservoir temperatures $T_H,T_C$ fixed while changing the per-rung energy spacing $\Delta E_k$, or (ii) hold the \,dimensionless\, transition biases---the quantities that directly enter the rates---fixed for each rung.

Option (i) is physically natural if one insists on the same reservoirs, but it also changes $\sigma_k=\tanh(\beta_H\Delta E_k/2)$ and $\omega_k=\tanh(\beta_C\Delta E_k/2)$ as the ladder is refined, which can obscure comparisons across $n$.

Option (ii) keeps $\sigma$ and $\omega$ fixed \,per rung\,, so that each adjacent transition is parameterized the same way as the two-state demon. In that case the ladder extension is a structural change rather than a hidden change of thermal bias. FIG.~\ref{fig:nSigmaOmega} shows $\Phi$ versus $n$ under this ``fixed-$\sigma,\omega$ per rung'' convention.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{images/n_state_phi_vs_n_sigmaomega.png}
  \caption{$\Phi$ versus number of demon states $n$ for a laddered demon where each rung uses the same dimensionless transition biases ($\sigma$ and $\omega$) as the baseline two-state case. This isolates the effect of enlarging the state space from the separate effect of changing $\tanh(\beta\Delta E/2)$ by shrinking energy spacings at fixed temperatures.}
  \label{fig:nSigmaOmega}
\end{figure}

Taken together, these viewpoints clarify why $\Delta E$ requires care in discussion: in an $n$-state ladder there is a distinction between a per-rung energy gap and the overall energy span of the ladder. What matters for rates---and thus for $\Phi$---is how those gaps enter the dimensionless combinations $\beta_H\Delta E_k$ and $\beta_C\Delta E_k$.

\section{Two-Bit Interacting Demon}
\label{sec:two_bit}

The original Maxwell refrigerator processes bits independently: the demon couples to a single bit and the only ``memory'' effect across time is the demon's own internal state. A natural extension is to allow the demon to couple to \emph{pairs} of bits. This generalization is motivated by two considerations. First, it introduces a minimal notion of structured input beyond a simple bias, because the incoming tape can carry correlations. Second, it allows cooperative transitions that can create or destroy those correlations while still respecting detailed balance with the reservoirs.

In the two-bit model used here, the demon remains a two-state system ($u/d$), but the tape is processed in adjacent pairs ($00,01,10,11$). Intrinsic transitions $u\leftrightarrow d$ exchange energy with the hot bath as before. Cooperative transitions now involve joint changes of the demon state and the current bit-pair, and are constructed to satisfy detailed balance with the cold bath. A key qualitative difference is that the demon can modify the pair distribution in ways that are invisible to single-bit statistics.

\subsection{Results}
FIG.~\ref{fig:twoBitMetrics} compares the two-bit demon to the single-bit demon on the same initial tape. Besides the information current $\Phi$ and the tape entropy change $\Delta S_B$, we track the change in pair correlation and the change in mutual information between the two bits in each pair. These quantities quantify the demon's ability to write \emph{structured} information to the tape.
...
We reproduced the behavior of Mandal \emph{et al.}\ \cite{mandal2013refrigerator} using a Gillespie simulation. The numerical phase diagrams match the exact solution, a sign that the modified Clausius bound is respected and that the implementation is consistent with the analytic results. We also verified the fast–demon limiting form
\begin{equation}
...
Generalizing the demon to an \(n\)-level energy ladder can boost \(\Phi\) by providing additional pathways for cooperative transitions. The quantitative comparison depends on what is held fixed as the ladder is refined (e.g., fixed temperatures versus fixed per-rung biases), but in either convention the same thermodynamic trade-off between heat flow and information processing persists: refrigeration \((Q_{c\to h}>0)\) should be accompanied by writing information, while erasure requires heat to flow down the gradient.

Overall, the extended models provide concrete routes to stronger autonomous refrigeration at fixed reservoirs: (i) stacking $K$ demons to realize an effective longer interaction without slowing the tape, (ii) enlarging the internal state space via energy ladders while being explicit about whether one holds temperatures or dimensionless biases fixed, and (iii) allowing multi-bit interactions to harness (and modify) correlations as an additional informational resource.
...
All code and plotting scripts used to generate the figures in this report are included with the submission under the \texttt{final-report/} directory. In particular, the implementations are organized under \texttt{final-report/code/} (two-state, stacked, $n$-state, and two-bit interacting demons) and the reproducible figure-generation scripts are under \texttt{final-report/scripts/}.
\end{document}
