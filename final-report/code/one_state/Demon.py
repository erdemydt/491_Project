...
class PhysParams:
    ...
    def __post_init__(self):
        ...
        if self.demon_n is not None and self.sigma_k is not None and self.omega_k is not None:
            deltaE_K = self.DeltaE / (self.demon_n - 1)
            self.Tc = deltaE_K / (2 * np.arctanh(self.omega_k))
            self.Th = deltaE_K / (2 * np.arctanh(self.sigma_k))
...
class Demon:
    ...
    def __init__(self, n: int, phys_params: PhysParams = None, init_state: str = 'd0', energy_distribution: str = "uniform"):
        ...
        print(f"Demon initialized with {n} states. Energy : {self.delta_e_values[0]}")
